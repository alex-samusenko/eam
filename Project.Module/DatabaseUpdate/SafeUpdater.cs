...
namespace Project.Module.DatabaseUpdate
{
    /// <summary>
    /// Class Updater.
    /// </summary>
    public class Updater : XafariModuleUpdater
    {
        ...
        /// </summary>
        private static void FillEmployeeShortIof(Session session)
        {
            var classInfo = session.GetClassInfo(typeof(Сотрудник));
            var uow = new UnitOfWork();
            var employees = uow.GetObjects(classInfo, CriteriaOperator.Parse("[ShortIOF] is null or [ShortIOF] = ''"), [], 0, 0, false, true);
        ...
        public static string FormatFullName(string fio)
        {
            ...
            var resFio = "";
            if (arrayFio.Length == 1)
                resFio = arrayFio[0];
            else if (arrayFio.Length == 2)
                resFio = string.Format("{1}. {0}", arrayFio[0], arrayFio[1][..1]);
            else if (arrayFio.Length > 2)
                resFio = string.Format("{1}.{2}. {0}", arrayFio[0], arrayFio[1][..1], arrayFio[2][..1]);
            return resFio;
        }
        /// <summary> Заполнение поля ShortIOF у всех работников с подготовкой
        /// </summary>
        private static void FillEmployeeShortIofWithPrepare(Session session)
        {
            var uow = new UnitOfWork();
            var classInfo = session.GetClassInfo(typeof(Сотрудник));
            var employees = uow.GetObjects(classInfo, CriteriaOperator.Parse("[ShortIOF] is null or [ShortIOF] = ''"),
                [], 0, 0, false, true);
            BatchWideDataHolder4Modification batchWideDataHolder = new(session);
            List<ModificationStatement> modificationStatement = [];
            foreach (Сотрудник employee in employees)
            {
                var propertyValueStore = new Dictionary<string, Object>(1)
